...
def fix_document_download_fixed():
    ...
    import re
    download_function_match = re.search(r'@enhanced_bp\.route\(\'/api/documents/\<doc_id\>/download\'.*?def download_document\(doc_id\):.*?except Exception as e:.*?return jsonify\(\{"success": False, "error": str\(e\)\}\), 500', content, re.DOTALL)
    
    if not download_function_match:
        logger.error("Couldn't find the download_document function in the file")
        return False
    
    old_function = download_function_match.group(0)
    
    # Create the new function with a more robust implementation
...
def download_document(doc_id):
    ...
    # Also fix the JavaScript for document downloads - add download function if missing
    ...
    if os.path.exists(js_file_path):
        ...
        if "downloadDocument" not in js_content:
            ...
            # Create download function to insert
            download_function = '''
    downloadDocument(docId) {
        ...
        document.body.removeChild(downloadLink);
            ...
            constructor_end_index = js_content.find("    initialize() {")
            if constructor_end_index != -1:
                js_content = js_content.slice(0, constructor_end_index) + download_function + "\n" + js_content.slice(constructor_end_index)
                
                # Add download button to document card UI
...
if __name__ == "__main__":
    try:
        if fix_document_download_fixed():
