#!/usr/bin/env python3
"""
Fix for multimodal integration issues.

This script creates a proper multimodal_integration.py file and ensures
it can be imported correctly by the web interface.
"""
...
def fix_multimodal_integration():
    """Fix multimodal integration issues."""
    # Create the multimodal_integration.py file
    multimodal_integration_path = os.path.join(
        os.path.dirname(__file__),
        'multimodal_integration.py'
    )
    
    logger.info(f"Creating multimodal integration at: {multimodal_integration_path}")
    
    with open(multimodal_integration_path, 'w') as f:
        f.write("""
\"\"\"
...
""")
    ...
    # Also create a symlink in the socratic_clarifier directory
    socratic_dir = os.path.join(
        os.path.dirname(__file__),
        'socratic_clarifier'
    )
    
    target_path = os.path.join(socratic_dir, 'multimodal_integration.py')
    
    if os.path.exists(target_path):
        logger.info(f"Removing existing multimodal integration at: {target_path}")
        os.remove(target_path)
    
    try:
        # Create a symlink (relative path)
        relative_path = os.path.relpath(multimodal_integration_path, os.path.dirname(target_path))
        os.symlink(relative_path, target_path)
        logger.info(f"Created symlink at: {target_path}")
    except Exception as e:
        logger.error(f"Failed to create symlink, copying file instead: {e}")
        # If symlink fails (e.g., on Windows), copy the file
        shutil.copy2(multimodal_integration_path, target_path)
        logger.info(f"Copied file to: {target_path}")
    
    # Fix imports in routes_multimodal.py
    ...
    if os.path.exists(routes_path):
        ...
        logger.info(f"Updated import in: {routes_path}")
    
    # Fix imports in document_manager.py
    doc_manager_path = os.path.join(
        os.path.dirname(__file__),
    ...
    if os.path.exists(doc_manager_path):
        ...
        updated_content = content.replace(
            "from socratic_clarifier.multimodal_integration import process_file",
            "try:\n                from socratic_clarifier.multimodal_integration import process_file\n            except ImportError:\n                # Import directly if not in socratic_clarifier\n                try:\n                    from multimodal_integration import process_file\n                except ImportError:\n                    logger.warning('Multimodal integration not available for text extraction')\n                    raise"
        )
        
        with open(doc_manager_path, 'w') as f:
            f.write(updated_content)
        
        logger.info(f"Updated import in: {doc_manager_path}")
...
if __name__ == "__main__":
    try:
        if fix_multimodal_integration():
            logger.info("Successfully fixed multimodal integration")
            sys.exit(0)
        else:
            logger.error("Failed to fix multimodal integration")
            sys.exit(1)
    except Exception as e:
        logger.error(f"Error fixing multimodal integration: {e}")
        sys.exit(1)
