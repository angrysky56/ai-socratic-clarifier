...
def check_sequential_thinking():
    """Check if MCP Sequential Thinking server is running."""
    try:
        import requests
        response = requests.get("http://localhost:3000")
        if response.status_code == 200:
            print("MCP Sequential Thinking server is running.")
            return True
        else:
            print(f"Sequential Thinking API error: {response.status_code}")
            return False
    except Exception as e:
        print(f"Error checking Sequential Thinking: {e}")
        return False

def start_sequential_thinking():
    """Start the MCP Sequential Thinking server if it's not running."""
    if check_sequential_thinking():
        return None
    
    try:
        print("Starting MCP Sequential Thinking server...")
        ...
        if check_sequential_thinking():
            print("MCP Sequential Thinking server started successfully.")
            return process
        else:
            print("Failed to start MCP Sequential Thinking server.")
            process.terminate()
            return None
    except Exception as e:
        print(f"Error starting Sequential Thinking server: {e}")
        return None
...
def main():
    ...
    # Start MCP Sequential Thinking server if needed
    st_process = start_sequential_thinking()
    if st_process:
        processes.append(st_process)
